...
from model import ft_net, ft_net_dense, ft_net_hr, ft_net_swin, ft_net_swinv2, ft_net_convnext, ft_net_efficient, ft_net_NAS, PCB, ClassBlock
...
parser.add_argument('--pretrained',default=0, type=int, help='0,1')
...
# Load model
#---------------------------
def load_network(network, class_names):
    save_path = os.path.join('./model',name,'net_%s.pth'%opt.which_epoch)
    pretrained = torch.load(save_path)
    model_dict = network.state_dict()
    pretrained = {k: v for k, v in pretrained.items() if (k in model_dict and 'classifier' not in k)}
...
def save_network(network, epoch_label):
    save_filename = 'new_net_%s.pth'% epoch_label
...
if opt.pretrained == 1:
    model = load_network(model, class_names)
    model.classifier = ClassBlock(2048, len(class_names), opt.droprate, linear=opt.linear_num, return_f = return_feature)
